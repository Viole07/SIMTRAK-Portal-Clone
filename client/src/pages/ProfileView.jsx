...
export default function ProfileView(){
  ...
  useEffect(()=>{ (async()=>{
    const [me,t,j,w,l]=await Promise.all([api.get("/auth/me"), api.get("/tasks"), api.get("/journals"), api.get("/weekly"), api.get("/leaves")]);
    setUser(me.data.user); setTasks(t.data.items||[]); setJournals(j.data.items||[]); setWeekly(w.data.items||[]); setLeaves(l.data.items||[]);
  })() },[]);
  if(!user) return <div className="grid h-[40vh] place-items-center">Loading…</div>;
  const p=user.profile||{}; const done=tasks.filter(x=>x.status==="Done").length; const ongoing=tasks.length-done;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="card card-pad">
        <div className="flex items-start gap-6">
          <div className="flex-1">
            ...
            <div className="text-sm text-slate-500 mt-1">NIN: <b>{user.ninNo||"—"}</b> · Status: <b>{user.status}</b></div>
          ...
          <aside className="w-56">
            <div className="rounded-2xl overflow-hidden border shadow-sm">
              <img className="w-full aspect-square object-cover" src={p.photoUrl || "https://via.placeholder.com/400x400?text=No+Photo"} />
            </div>
            <div className="mt-3 grid gap-2">
              <Link to="/profile" className="btn-ghost text-center">Edit Profile</Link>
              <Link to="/photo" className="btn-ghost text-center">Update Photo</Link>
            </div>
          </aside>
        </div>
      </div>
      ...
      <div className="card card-pad">
        <div className="grid md:grid-cols-2 gap-4">
          {[
            ...
            <div key={k}><div className="text-xs uppercase text-slate-500">{k}</div><div className="font-medium">{v || "—"}</div></div>
          ))}
        </div>
      </div>
      ...
      <div className="grid xl:grid-cols-2 gap-4">
        <Mini title="Recent Tasks" to="/app/tasks"
...
function Stat({title,value,sub,Icon,to}){
  return (
    <Link to={to} className="card card-pad hover:shadow-md transition">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-xl bg-slate-100"><Icon size={18}/></div>
        <div>
